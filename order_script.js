document.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- PROMO 5% ---
  const PROMO = { percent: 5, tag: "[PROMO5]" };

function isPromoEligible(dishes) {
  const hasMeatSoup = !!dishes.soup && (dishes.soup.kind === "meat");
  const hasDessert = !!dishes.dessert;
  return hasMeatSoup && hasDessert;
}


  function applyPromo(total) {
    return Math.round(total * (1 - PROMO.percent / 100));
  }
  ...
  function createOrderDishCard(dish) {
    ...
    const imgSrc = dish.image && String(dish.image).trim()
      ? dish.image
  ...
  function updateOrderSummary() {
    ...
    const promoOn = isPromoEligible(selectedDishes);
    const finalCost = promoOn ? applyPromo(totalCost) : totalCost;

    summaryHTML += `
      <p style="font-weight: bold; margin-top: 20px;">Стоимость заказа</p>
      ${promoOn ? `<p style="text-decoration: line-through; opacity: .75;">${totalCost}₽</p>` : ""}
      <p style="font-size: 1.2em; font-weight: bold;">
        ${finalCost}₽ ${promoOn ? `(−${PROMO.percent}%)` : ""}
      </p>
    `;
  ...
  async function loadAllDishesData() {
    return await fetchJson(DISHES_URL);
  }
  ...
  function buildOrderPayload() {
    ...
    if (promoOn && !comment.includes(PROMO.tag)) {
      comment = (comment ? comment + " " : "") + PROMO.tag;
    }
  ...
  async function createOrder(e) {
    ...
    try {
      await fetchJson(withKey(ORDERS_URL), {
        method: "POST",
      ...
      localStorage.removeItem("promo5");

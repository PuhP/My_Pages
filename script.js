document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateStickyPanel() {
    ...
    // price line
    orderCostElement.textContent = promoOn
      ? `${finalCost}₽ (−${PROMO.percent}%)`
      : `${totalCost}₽`;

    // save promo flag
    localStorage.setItem('promo5', promoOn ? '1' : '0');
  ...
  async function init() {
    ...
    if (allDishes.length > 0) {
      ...
      const soupContainer = document.getElementById('soup_container');
      if (soupContainer) soupContainer.innerHTML = message;
    }
  }

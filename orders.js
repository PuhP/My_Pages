document.addEventListener("DOMContentLoaded", () => {
  // ВАЖНО: сюда подставь свой ключ локально
  const API_KEY = "a0cd61c9-08ca-4666-adc5-cfa927d3e73b";

  // Base URL из методички (Netlify/GitHub Pages)
  ...
  let dishesMap = new Map(); // id -> dish
  let orders = [];           // cached list
  ...
  function showToast(message, type = "ok") {
    ...
    toastEl.style.border = type === "error" ? "2px solid rgb(231, 43, 43)" : "2px solid rgb(75, 39, 4)";
  ...
  function orderCompositionText(order) {
    ...
    const names = ids
      .map((id) => dishesMap.get(Number(id))?.name)
      .filter(Boolean);
  ...
  async function loadOrders() {
    ...
    // сортировка по убыванию даты оформления: сначала новые
  ...
  async function refresh() {
    ...
    try {
      if (API_KEY === "YOUR_API_KEY") {
        throw new Error("Укажи API_KEY в js_files/orders.js (свой персональный ключ).");
      }
  ...
  async function saveEdit(e) {
    ...
    if (editType.value === "by_time") {
      payload.delivery_time = editTime.value; // "HH:MM"
    } else {
      // можно не отправлять delivery_time вовсе, но отправка null тоже обычно ок
    ...
    try {
      ...
      // обновляем локально и перерисовываем
      orders = orders.map((o) => (Number(o.id) === Number(id) ? { ...o, ...updated } : o))
